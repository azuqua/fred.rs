...
impl fmt::Debug for RedisCommandKind {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    if self.is_cluster_command() {
      write!(f, "{} {}", self.to_str_debug(), self.cluster_args().unwrap_or("".into()))
    }else if self.is_client_command() {
      write!(f, "{} {}", self.to_str_debug(), self.client_args().unwrap_or("".into()))
    }else {
...
impl RedisCommandKind {
  ...
  /// Debugging version of `to_str` that will never panic.

...
pub trait RedisClientBorrowed {
  ...
  fn psubscribe<K: Into<MultipleKeys>>(&self, patterns: K) -> Box<Future<Item=(), Error=RedisError>>;

  fn punsubscribe<K: Into<MultipleKeys>>(&self, patterns: K) -> Box<Future<Item=(), Error=RedisError>>;
